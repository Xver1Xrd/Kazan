        ...
        <div className="mt-12 grid gap-10 md:grid-cols-3">
          {CHECKLIST.map((group, i) => (
            <Reveal key={group.category} delay={i * 0.1}>
              <h3 className="text-lg font-semibold text-[#1f2a1d] dark:text-white mb-4">{group.category}</h3>
              <ul className="flex flex-col gap-3">
                {group.items.map((item) => (
                  <li key={item} className="flex items-start gap-2 text-sm text-[#4b5b47] dark:text-white/60 leading-relaxed">
                    <Check className="w-4 h-4 text-[#4b7a5a] dark:text-[#a9cbad] mt-0.5 flex-shrink-0" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
